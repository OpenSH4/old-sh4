...
static int qf9700_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	udelay(10 * 1000);	// at least 10ms, here 20ms for safe
	qf_write_reg(dev, PRR, 0);
	udelay(1 * 1000);	// at least 1ms, here 2ms for reading right register
...
static int qf9700_link_reset(struct usbnet *dev)
{
	...
	devdbg(dev, "link_reset() speed: 10 duplex: half",
