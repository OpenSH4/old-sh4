...
wget http://dbs-clan.de/argus/mtdiltv/mtd3-opti-st.bin -O /media/iltv/mtd3-opti-st.bin
sleep 2
# Check ob das file gedownloadet wurde
if [ ! -e /media/iltv/mtd3-opti-st.bin ]; then
...
# set new md5 Status
iltvmd5=`cat /media/iltv/md5check | grep mtd3-opti-st.bin | awk '{ print $2 }'`
...
echo ""
	...
	#nandwrite /dev/mtd3 /media/iltv/mtd3-opti-st.bin
	dd if=/media/iltv/mtd3-opti-st.bin of=/dev/mtdblock3 bs=1024
else
